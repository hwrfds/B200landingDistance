...
st.markdown("### Step 1: Baseline Landing Distance")
st.write(f"Pressure Alt: **{press_alt} ft**, OAT: **{oat} °C**")
...
def lookup_tbl2(df, base, w):
    if 12500 not in df.columns:
        st.error("Column 12500 lb not found in weightadjustment.csv.")
        st.stop()
    ref = df[12500]
    valid = ref[ref <= base]
...
st.write(f"Nearest row @ 12 500 lb: **{lookup_tbl2(df2, baseline, 12500):.0f} ft**")
...
def lookup_tbl3(df, refd, ws):
    ...
    if ws not in df.columns:
        st.error(f"Selected wind {ws} kt not found in wind adjustment table.")
        st.stop()
...
st.write(f"Wind Speed: **{wind:+.0f} kt** → Δ: **{delta_wind:+.0f} ft**")
st.success(f"Wind-adjusted distance: **{wind_adj:.0f} ft**")
