...
try:
    wt_cols = [int(str(w).strip()) for w in raw2.iloc[0]]
except Exception as e:
    st.error(f"Could not parse weight headers in Table 2: {e}")
    st.stop()
...
st.write(f"Baseline: **{baseline:.0f} ft**  \nSelected Weight: **{weight} lb**")
st.success(f"Adjusted for weight: **{weight_adj:.0f} ft**")
...
try:
    ...
    st.error(f"Could not parse wind headers in Table 3: {e}")
    st.stop()
...
st.write(f"Wind Speed: **{wind:+.0f} kt** → Adjustment Δ: **{delta_wind:+.0f} ft**")
st.success(f"Distance after wind: **{wind_adj:.0f} ft**")
...
try:
    ...
    st.error(f"Could not parse obstacle headers in Table 4: {e}")
    st.stop()
...
st.success(f"Final distance over 50 ft obstacle: **{obs50:.0f} ft**")
